...
function usage {
    ...
    echo "-D, --disk"
    echo "   Device number to check temperature"
    echo "-W, --warn"
...
if ! [[ $DISKDEV =~ $numre ]]; then
	echo "Warning disk no $DISKDEV is not an integer"
	exit ${STATE_UNKNOWN}
fi
...
TEMPDEG=$(sudo isdct show -sensor -intelssd ${DISKDEV}| grep 'Temperature - Celsius'|awk '{print $5}')
