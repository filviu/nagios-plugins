...
if [ $pool = "ALL" ]
then
  POOLS=($(sudo zpool list -Ho name))
  p=0
  for POOL in ${POOLS[*]}
  do 
    CAPACITY=$(sudo zpool list -Ho capacity $POOL | awk -F"%" '{print $1}')
    HEALTH=$(sudo zpool list -Ho health $POOL)
...
## Check single pool
  ...
  then 
    # Check with thresholds
    if [ $HEALTH != "ONLINE" ]; then echo "ZFS POOL $pool health is $HEALTH|$pool=${CAPACITY}%;${warn}%;${crit}%"; exit ${STATE_CRITICAL}
    elif [[ $CAPACITY -gt $crit ]]; then echo "ZFS POOL $pool usage is CRITICAL (${CAPACITY}%|$pool=${CAPACITY}%;${warn}%;${crit}%)"; exit ${STATE_CRITICAL}
    elif [[ $CAPACITY -gt $warn && $CAPACITY -lt $crit ]]; then echo "ZFS POOL $pool usage is WARNING (${CAPACITY}%)|$pool=${CAPACITY}%;${warn}%;${crit}%"; exit ${STATE_WARNING}
    else echo "ZFS POOL $pool OK |$pool=${CAPACITY}%;${warn}%;${crit}%"; exit ${STATE_OK}
    fi
  else
    # Check without thresholds
    if [ $HEALTH != "ONLINE" ]
